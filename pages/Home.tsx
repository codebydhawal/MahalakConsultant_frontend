...
import { Project, MediaItem, SiteConfig } from '../types';
...
export const Home: React.FC<{ config: SiteConfig }> = ({ config }) => {
  const [selectedService, setSelectedService] = useState<any>(null);
  const [featuredProjects, setFeaturedProjects] = useState<ProjectResponse[]>([]);
  const [currentProjectIndex, setCurrentProjectIndex] = useState(0);

  const [featuredMedia, setFeaturedMedia] = useState<MediaResponse[]>([]);
  ...
  const nextMedia = () => {
    setCurrentMediaIndex((prev) => (prev + 1) % featuredMedia.length);
  };

  const prevMedia = () => {
    setCurrentMediaIndex((prev) => (prev - 1 + featuredMedia.length) % featuredMedia.length);
  };
  ...
  useEffect(() => {
    ...
    const loadProjects = async () => {
      ...
      try {
        ...
        const response = await ProjectService.getRandomProjects();
      ...
      <section className="py-24 bg-white border-t border-stone-50 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className=" flex justify-end gap-4 mb-8">
            <button onClick={prevMedia} className="w-16 h-16 rounded-full border border-stone-200 flex items-center justify-center text-stone-600 hover:bg-stone-900 hover:text-white transition-all">
              <i className="fa-solid fa-arrow-left"></i>
            </button>
            <button onClick={nextMedia} className="w-16 h-16 rounded-full border border-stone-200 flex items-center justify-center text-stone-600 hover:bg-stone-900 hover:text-white transition-all">
              <i className="fa-solid fa-arrow-right"></i>
            </button>
          </div>
          <div className="flex md:grid md:grid-cols-3 gap-10 overflow-x-auto md:overflow-visible no-scrollbar -mx-4 px-4 snap-x snap-mandatory">
            {featuredMedia.map(item => (
              <a href={item.videoUrl} target="_blank" rel="noopener noreferrer" key={item.mediaId} className="group shrink-0 w-[300px] md:w-auto snap-center">
                <div className="relative aspect-video rounded-3xl overflow-hidden mb-8 shadow-lg">
                  <img src={item.thumbnailImageUrl} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000" />
                ...
                <Link
                  to={item.videoUrl}
                  target="_blank"
                  className="inline-flex items-center gap-2 mt-4 text-amber-700 font-bold text-xs uppercase tracking-widest hover:underline"
