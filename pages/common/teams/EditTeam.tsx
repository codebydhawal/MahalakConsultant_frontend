...
const EditTeam = () => {
    ...
    const loadTeamMember = async (teamId: string) => {
        try {
            ...
            const member: TeamResponse = response.data.data;

            setFullName(member.fullName);
            setDesignation(member.designation);

            setBio(member.shortBio);

            setEmail(member.email);
            setPhone(member.phoneNumber);

            setLinkedIn(member.linkedInUrl);
            setInstagram(member.instagramUrl);
            setFacebook(member.facebookUrl);
            setTwitter(member.twitterUrl);

            setDisplayOrder(member.displayOrder);

            setPreview(member.profileImageUrl);
